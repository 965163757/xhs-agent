...
> 方向：编辑工作室·克制黑白 · Refined detail pass
...
- 动效像**编辑部里的实时状态**，不是营销页炫技：线条绘制、稿件入场、工具调用扫描、任务进度、雷达描边、图层选区呼吸。
- Chrome 底部用 1px 红色 sweep 表示页面处于 live 状态；不使用大面积 loading gradient。
- Tool ledger / event feed 用轻微横向扫描表达“Agent 正在工作”，让用户看到可审计过程，而不是只看到 spinner。
- Chat 回复使用细红 caret，表达流式输出；任务卡顶部使用 2px progress rail，表达后台任务仍在跑。
- Image Lab 的选区框和控制点做低频呼吸，强调可编辑对象；Analytics 条形图使用 left-origin grow，符合数据出现的方向。
- 所有动效必须有 `prefers-reduced-motion` 降级；默认时长控制在 180ms-4.8s，避免漂浮感和赛博感。
...
- **工具调用 ledger**：Chat / Tasks / Diagnose 都用同一套 mono 时间轴，显示 tool name、状态、耗时、SSE 恢复线索。让 Agent 的“工作中”从 spinner 变成可审计过程。
- **稿件批注**：Article Detail 中加入 editor note、diff、小型质检条，强化“稿件”而不是“表单”的心智。
- **图片实验室选区控制点**：Image Lab 的 mask、edge、feather 三个参数露出，避免只是普通上传页。
